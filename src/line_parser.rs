...
fn parse_param<'a>(i: &'a str) -> IResult<&'a str, ParserResult, VerboseError<&'a str>> {
	...
	delimited(
	    ...
	    alt((	
		map(parse_float, |f| ParserResult::Scalar(f)),
		map(separated_pair(tag("ramp"), tag(" "), separated_list0(tag(" "), parse_float)), |v| ParserResult::Ramp(v.1)),
		map(separated_pair(tag("bounce"), tag(" "), separated_list0(tag(" "), parse_float)), |v| ParserResult::Bounce(v.1)),
		map(separated_pair(tag("choose"), tag(" "), separated_list0(tag(" "), parse_float)), |v| ParserResult::Choose(v.1)),
		map(separated_pair(tag("cycle"), tag(" "), separated_list0(tag(" "), parse_float)), |v| ParserResult::Cycle(v.1)),
	    )),	    
	    tag("]"))
    ))(i)
}
...
fn parse_string<'a>(i: &'a str) -> IResult<&'a str, ParserResult, VerboseError<&'a str>> {
    map(
        ...
        |desc_str: &str| ParserResult::String(desc_str.to_string()),
    )(i)
}
...
mod tests {
    ...
    #[test]
    fn test_line_parser() {
	let result = parse_line("img forest.jpg pos 0.2 [0.2] [bounce 0.2 0.3] [ramp 0.2 0.3] [choose 0.2 0.3 400]").unwrap();
	println!("{}", result.0);
